...
| Explainable recommendations | Real recommendation rows with scores, confidence, reasons, usable URLs/IDs | **Pass**: up to 50 persisted fresh rows with scores, confidence, artist-affinity reasons, and YouTube Music URLs/IDs |
| Automatic playlist generation | Successful real sync creates a persisted local plan without prompting | **Pass**: local 50-target playlist preview is persisted and regenerated without prompting |
